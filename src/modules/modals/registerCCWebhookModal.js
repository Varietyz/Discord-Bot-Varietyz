            ...
            try {
                if (targetChannel.isTextBased()) {
                    if (targetChannel.isTextBased() && 'send' in targetChannel) {
                        ...
                        logger.warn(`⚠️ The target channel ${targetChannelId} is not a text-based channel or does not support sending messages.`);
                    }
                } else {
        ...
        if (error.message.includes('SQLITE_CONSTRAINT: UNIQUE constraint failed: clanchat_config.secret_key')) {
            ...
            if (existingEntry) {
                const embed = new EmbedBuilder()
                    ...
                    .setDescription('This secret key is already associated with an existing ClanChat webhook.')
                    .addFields(
                        { name: '🔑 **ClanChat Key**', value: `\`${existingEntry.clanchat_key}\``, inline: true },
                        { name: '🏛 **Clan Name**', value: `\`${existingEntry.clan_name}\``, inline: true },
                        { name: '🔗 **Webhook URL**', value: `\`\`\`${existingEntry.webhook_url}\`\`\``, inline: false },
                        { name: '📢 **Channel**', value: `<#${existingEntry.channel_id}>`, inline: true },
                        { name: '🌍 **Endpoint URL**', value: `\`${existingEntry.endpoint_url}\``, inline: true },
