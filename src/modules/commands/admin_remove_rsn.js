...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setName('target')
                .setDescription(
                    'The guild member whose RSN you want to remove.'
                )
                .setRequired(true)
        ...
        .addStringOption((option) =>
            option
                .setName('rsn')
                .setDescription('The RSN to remove.')
                .setRequired(true)
                .setAutocomplete(true)
        ),
    ...
    async execute(interaction) {
        try {
            ...
            logger.info(
                `Administrator ${interaction.user.id} attempting to remove RSN "${rsnInput}" from user ${targetUserID}`
            );
            ...
            const confirmationRow = new ActionRowBuilder().addComponents(
                new ButtonBuilder()
                    .setCustomId('confirm_remove_rsn')
                    .setLabel('Confirm')
                    .setStyle(ButtonStyle.Danger),
                new ButtonBuilder()
                    .setCustomId('cancel_remove_rsn')
                    .setLabel('Cancel')
            ...
            collector.on('collect', async (i) => {
                if (i.customId === 'confirm_remove_rsn') {
                    ...
                    logger.info(
                        `RSN "${rsnInput}" successfully removed from user ${targetUserID} by admin ${interaction.user.id}`
                    );
            ...
            logger.error(
                `Error executing /admin_remove_rsn command: ${error.message}`
            );
    ...
    async autocomplete(interaction) {
        ...
        try {
                        ...
                        const rsnDisplay =
                            matchingRsns.length > 0
                                ? matchingRsns
                                    ...
                                    .join(', ') // Display all matching RSNs
                                : 'No matching RSNs';
            ...
            logger.error(
                `Error in autocomplete for /admin_remove_rsn: ${error.message}`
            );
