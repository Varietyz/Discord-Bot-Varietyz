...
async function syncEmojis(interaction) {
    try {
        ...
        let addedEmojis = 0;
        let deletedEmojis = 0;
        const skillBossEmojis = await db.image.getAll(
            'SELECT file_name, file_path FROM emojis'
        );
        logger.info(
            `🔄 Retrieved ${skillBossEmojis.length} skill/boss emojis from DB.`
        );
        ...
        for (const [baseName, filePath] of skillBossEmojiMap) {
            ...
            try {
             ...
             SET emoji_key = excluded.emoji_key, emoji_name = excluded.emoji_name, emoji_format = excluded.emoji_format, animated = excluded.animated`,
                ...
                logger.error(
                    `❌ Failed to upload skill/boss emoji (${baseName}): ${error.message}`
                );
            }
        }
            ...
            if (skillBossEmojiMap.has(storedKey.substring(6))) {
                ...
                if (!fs.existsSync(fileFullPath)) {
                    try {
                        ...
                        await sleep(1500);
                        logger.info(
                            `🗑 Removed outdated skill/boss emoji (key: ${storedKey}, file missing)`
                        );
                    } catch (error) {
                        logger.warn(
    ...
    try {
        ...
        collector.on('collect', async (i) => {
            if (i.customId === 'confirm_delete_emojis') {
                ...
                await i.update({
                    content: '❌ Emoji deletion **cancelled**.',
                    components: [],
                });
            }
            collector.stop();
        ...
        await interaction.editReply(
            '❌ **Error:** Could not process deletion confirmation.'
        );
    }
}
    ...
    try {
        ...
        for (const emoji of guildEmojis.values()) {
            try {
                ...
                logger.warn(
                    `⚠️ Failed to delete emoji (${emoji.name}): ${error.message}`
                );
            }
        }
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) =>
            subcommand
                .setName('delete')
                .setDescription(
                    'ADMIN: Delete ALL emojis from the server (requires confirmation).'
                )
        ),
