    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) =>
            subcommand
                .setName('new')
                .setDescription(
                    'End the current Bingo event manually and start a new one.'
                )
        )
        .addSubcommand((subcommand) =>
            subcommand
                .setName('update')
                .setDescription('Update the current Bingo event.')
        ),
    async execute(interaction) {
        try {
            const emoji = await getEmojiWithFallback(
                'emoji_clan_loading',
                'loading...'
            );
  ...
  `);
            ...
            case 'new':
                interaction.editReply({
                    content: `${emoji} Ending current event and initializing a new one... please wait.`,
                });
                ...
                interaction.editReply({
                    content: '🏁 Bingo event has been ended via auto-transition.',
                });
                break;
            case 'update':
                interaction.editReply({
                    content: `${emoji} Updating bingo progression for everyone... please wait.`,
                });
                await updateBingoProgress(client);
                ...
                await autoTransitionEvents();
                interaction.editReply({
                    content: '🏁 Bingo event has been updated!',
                });
                break;
            default:
