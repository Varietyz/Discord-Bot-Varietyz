...
const {
    ...
    getPlayerTasks,
    getEventId,
} = require('../../services/bingo/bingoImageGenerator');
const WOMApiClient = require('../../../api/wise_old_man/apiClient');
const { savePlayerDataToDb } = require('../../services/playerDataExtractor');
const {
    getLastFetchedTime,
    setLastFetchedTime,
} = require('../../utils/fetchers/lastFetchedTime');
const {
    getDataAttributes,
    getDataColumn,
    upsertTaskProgress,
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setName('rsn')
                .setDescription('The RSN to view (optional).')
                .setAutocomplete(true)
                .setRequired(false)
        ),
    async execute(interaction) {
        try {
            const emojiTeam = await getEmojiWithFallback(
                'emoji_clan_loading_team',
                'Loading...'
            );
            const emojiSolo = await getEmojiWithFallback(
                'emoji_clan_loading_solo',
                'Loading...'
            );
    ...
    `,
                ...
                if (userRows.length === 0) {
                    return interaction.editReply({
                        content:
              '❌ You are either not registered or not an active clan member. Join the clan and register your RSN to participate in Bingo.',
                        flags: 64,
                    });
                }
            }
            ...
            if (teamMember) {
                logger.info(
                    `[BingoCards] RSN ${teamMember.rsn} is in a team (${teamMember.team_name}). Updating team member data...`
                );
    ...
    async autocomplete(interaction) {
        try {
            ...
            const choices = results
                .map((row) => ({ name: row.rsn, value: row.rsn }))
                .slice(0, 25);
...
async function buildActivePatternsEmbed(eventId, numRows = 3, numCols = 5) {
    ...
    const patternRows = await db.getAll(
        'SELECT pattern_key, created_at FROM bingo_pattern_rotation WHERE event_id=?',
        [eventId]
    );

    const dynamicPatternFields = await buildActivePatternFields(
        patternRows,
        numRows,
        numCols
    );
...
const generateBingoEmbed = async (boardId, player) => {
    ...
    const fetchPlayerLeaderboardData = async (player) => {
      ...
      await computeIndividualPartialPoints(boardId, player_id, true);
        ...
        for (const task of taskProgress) {
            if (task.status === 'completed') {
                if (!emojiCache[task.parameter]) {
                    emojiCache[task.parameter] = await getEmojiWithFallback(
...
async function paginateIndividualCards(interaction, boardId, userRows) {
    ...
    const showCard = async (index) => {
        if (index < 0 || index >= total) {
            return interaction.editReply({
                content: '❌ No more Bingo cards to display.',
                components: [],
            });
        }
...
async function generateTeamCard(interaction, boardId, teamId, teamName) {
    ...
    try {
        ...
        } = await computeTeamPartialPoints(eventId, teamId);
        ...
        teamTotalOverallPartial > 0
            ? ((userOverallPoints / teamTotalOverallPartial) * 100).toFixed(2)
            : '0.00';

            const profileLink = await getPlayerLink(member.rsn);

            memberLines.push(
                `> - ${profileLink} — \`${userShare}%\` ${progressEmoji} — \`${actualPoints} pts\` ${pointsEmoji}`
            );
        }
        ...
        teamProgress.push({
            team_name: teamName,
            overall, 
            partial_points: actualTeamPoints, 
        ...
        const embed = new EmbedBuilder()
            .setTitle(`${trophyEmoji} Team Card — Bingo Event #${eventId}`)
            .setColor(0xf1c40f)
