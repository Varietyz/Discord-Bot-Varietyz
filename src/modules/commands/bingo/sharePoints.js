...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setName('rsn')
                .setDescription('The RSN to share points with.')
        ...
        .addStringOption((option) =>
            option
                .setName('type')
                .setDescription('The type of points to transfer.')
                .setRequired(true)
                .setAutocomplete(true)
        )
        .addIntegerOption((option) =>
            option
                .setName('amount')
                .setDescription(
                    'The number of points to share (must be within available balance).'
                )
                .setAutocomplete(true)
                .setRequired(true)
        ),
        ...
        try {
            ...
            if (!sender) {
                return interaction.editReply({
                    content:
            '❌ You are not registered in the system or do not have points in this category.',
                });
            }
            ...
            if (!recipient) {
                return interaction.editReply({
                    content: `❌ RSN **${recipientRSN}** is not a registered clan member.`,
                });
            }
            ...
            await updatePlayerPoints(sender.player_id, pointType, -amount); 
            await updatePlayerPoints(recipient.player_id, pointType, amount); 
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(0x00ff00)
                .setDescription(
                    `You have successfully sent ⭐ **\`${amount}\`** pts from **${pointType}** to ${playerLink}.\n\n`
                )
                .addFields({
                    name: '📊 View Your Transactions',
                    value: 'Use `/wallet` to check your balance and past transactions!',
                })
                .setTimestamp();
                ...
                try {
                    ...
                    if (recipientUser) {
                        const recipientDM = new EmbedBuilder()
                            ...
                            .setColor(0x3498db)
                            .setDescription(
                                `🎉 You have received ⭐ **\`${amount}\`** pts towards ** ${pointType}** from **${sender.rsn}**!`
                            )
                            .addFields({
                                name: '🔍 Check Your Balance',
                                value:
                  'Use `/wallet` in the server to see your updated points!',
                            })
                            .setTimestamp();
                    ...
                    logger.warn(
                        `⚠️ Could not send DM to user ${recipient.discord_id}: ${dmError.message}`
                    );
                }
            }
    ...
    async autocomplete(interaction) {
        try {
            ...
            if (focusedOption.name === 'rsn') {
                ...
                const choices = results
                    .map((row) => ({ name: row.type, value: row.type }))
                    .slice(0, 25);
                await interaction.respond(choices);
