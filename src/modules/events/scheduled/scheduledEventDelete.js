...
module.exports = {
    ...
    async execute(event) {
        ...
        try {
            logger.info(
                `🗑️ [ScheduledEventDelete] Scheduled event "${event.name}" was deleted.`
            );
            ...
            logger.info(
                `📋 Successfully logged scheduled event deletion: "${event.name}"`
            );
        } catch (error) {
            logger.error(
                `❌ Error logging scheduled event deletion: ${error.message}`
            );
        }
    },
};
