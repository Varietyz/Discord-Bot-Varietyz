 ...
 * Main, Messages, Image Cache, and Guild. It ensures:
...
async function handleDbError(error, dbName, client) {
    ...
    try {
        ...
        const existingError = await dbFunctions.guild.getOne('SELECT * FROM error_logs WHERE error_message = ? AND last_occurred > DATETIME(\'now\', \'-1800 seconds\')', [error.message]);
        if (existingError) {
            await dbFunctions.guild.runQuery('UPDATE error_logs SET occurrences = occurrences + 1, last_occurred = CURRENT_TIMESTAMP WHERE id = ?', [existingError.id]);
        } else {
            await dbFunctions.guild.runQuery('INSERT INTO error_logs (error_message, error_stack) VALUES (?, ?)', [error.message, error.stack || null]);
        }
    } catch (logError) {
...
async function setConfigValue(key, value) {
    ...
    await dbFunctions.main.runQuery('INSERT OR REPLACE INTO config (key, value) VALUES (?, ?)', [key, valStr]);
}
...
module.exports = {
    ...
    setConfigValue,
    messages: {
        runQuery: (...args) => initializationPromise.then(() => dbFunctions.messages.runQuery(...args)),
        getAll: (...args) => initializationPromise.then(() => dbFunctions.messages.getAll(...args)),
        getOne: (...args) => initializationPromise.then(() => dbFunctions.messages.getOne(...args)),
    },
    image: {
